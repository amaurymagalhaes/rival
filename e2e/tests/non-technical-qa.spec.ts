...
test('non-technical QA checklist executable cases', async ({ page }) => {
  ...
  const runCase = async (id: string, fn: () => Promise<void>, note?: string) => {
    try {
      await fn();
  ...
  await runCase('LIKE-02', async () => {
    ...
    await page.reload();
    const reloadedCount = page.locator('button:has(svg.lucide-heart)').first().locator('span').first();
